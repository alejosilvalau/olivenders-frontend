  ...
  editSchool(): void {
    if (this.selectedSchool) {
      ...
      this.schoolService.update(this.selectedSchool.id, schoolData).subscribe({
        ...
        error: (err: any) => {
          this.alertComponent.showAlert(err.error.message || 'Error updating the school', AlertType.Error);
        }
      });
      this.schoolForm.reset();
    }
  ...
  removeSchool(): void {
    if (this.selectedSchool) {
      this.schoolService.remove(this.selectedSchool.id).subscribe({
        ...
        error: (err: any) => {
          this.alertComponent.showAlert(err.error.message || 'Error deleting the school', AlertType.Error);
        }
      });
      this.schoolForm.reset();
    }
